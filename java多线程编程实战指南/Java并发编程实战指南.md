...
如果一个类在单线程环节下能够运行正常，并且在多线程环境下，在使用时不做任何改变也能正常运行，那么我们就称其为线程安全的；相应的，如果一个类在单线程环境下能够正常运行而多线程环境下无法正常运行，那么此类就是非线程安全的。如果一个类能产生竞态，那么它就是非线程安全的
...
**不可分割的另一个含义**是，设O1和O2是访问共享变量V的两个原子操作，这两个操作并非都是读操作。那么一个线程执行O1期间（开始执行而未执行完毕），其它线程无法执行O2。**也就是说，访问同一组共享变量的原子操作时不能被交错的**，这就排除了一个线程执行一个操作期间另外一个线程读取或更新访问共享变量而导致的干扰
...
对于使用volatile关键字修饰的long/double变量的**写操作具有原子性**，但是它不能保障其他操作（比如read-modify-write操作和check-then-act操作）的原子性
...
初始情况下生产者和消费者各自创建一个空的缓冲区。消费者线程执行exchange方法时将参数x指定为一个空的或者已经使用过的缓冲区，生产者线程执行exchange时将参数指定为一个已经填充完毕的缓冲区。**当两个线程都执行到exchange(V x)方法时，exchange(V x)的返回值是对方执行该方法时指定的参数x的值**
...
由于线程和任务之间可以是一对多的关系，一个线程可以先后执行多个任务，因此线程特有对象相当于一个线程所执行的多个任务之间的共享变量。如果线程特有对象随着执行任务而改变，那么下个任务看到的是来自上一个任务留下的数据，这个数据可能与该任务不匹配，导致数据错乱。因此由此出发，确保每个任务的处理逻辑被执行前相应的线程特有对象的状态不收前一个被执行任务的影响，可以在一个任务开始前调用ThreadLocal.set(T value)方法或者重置特有对象的状态
...
**Collections.synchronizedX(X可以是Set、List、Map等)方法就是使用装饰器模式**将指定的非线程安全集合对象对外暴露为线程安全对象。该方法的返回值是指定集合对象的外包装对象，也称为**同步集合**。例如Collections.synchronizedMap根据指定的非线程安全Map接口实现类返回一个相应的外包装对象
...
从Java6/Java7开始，Java虚拟机对内部锁的实现进行了一些优化，包括**锁消除、锁粗化、偏向锁以及适应性锁**，这些优化仅在Java虚拟机server模式下起作用（通过虚拟机-server参数开启）

#### 12.1.1 锁消除

锁消除是JIT编译器对内部锁的具体实现所做的一种优化。在动态编译同步块的时候，JIT编译器可以借助一种被称为**逃逸分析的技术**来判断同步块所使用的锁对象是否只能被一个线程访问而没有被发布到其他线程，**如果同步块所使用的锁对象通过这种分析被证实只能够被一个线程访问，那么JIT编译器在编译这个同步块的时候并不生成synchronized所表示的锁的申请与释放对应的机器码从而达到消除锁的使用**

**锁消除优化能否被实施还取决于被调用的同步方法（或者带同步块的方法）是否能被内联（即将调用的方法替换为方法体本身）**

#### 12.1.2 锁粗化

锁粗化是JIT编译器对内部锁的具体实现所做的一种优化。**对于相邻的几个同步块，如果这些同步块使用的是同一个锁实例，那么JIT编译器会将这些同步块合并为一个大同步块，从而避免一个线程反复申请、释放同一个锁的开销**

相邻的两个同步块之间如果存在其他语句，也不一定会阻碍JIT编译器执行锁粗化优化，因为JIT编译器可能在执行锁粗化时将这些语句挪到后一个同步块的临界区中（重排序）

锁粗化可能导致一个线程持有一个锁的时间变长，从而使得其他同步在该锁上的其他线程申请锁的等待时间延长

#### 12.1.3 偏向锁

偏向锁时Java虚拟机对锁的实现所作的一种优化。因为大多锁并没有被争用，并且这些锁在整个生命周期内至多只会被一个线程持有。然而**Java虚拟机在实现monitorenter字节码和monitorexit字节码时需要借助一个原子操作（CAS），这个操作的代价相对比较昂贵**。

基于上述原因，Java虚拟机会为每个对象维护一个**偏好**，即一个对象对应的内部锁第1次被一个线程获得，那么这个线程就会被记录为该对象的偏好线程。这个线程之后无论是再次申请该锁还是释放该锁，都无需借助原先昂贵的原子操作，从而减少了锁的申请与释放的开销

一个锁没有被争用并不代表只有一个线程访问，当一个对象的偏向线程以外的其他线程申请该对象的内部锁，Java虚拟机需要收回该对象对原偏好线程的“偏好”并重新设置该对象的偏好线程。这种偏好的收回与重新分配过程的代价也是比较昂贵的，如果程序中存在较多的锁争用的情况，此时偏好设置就会被放大。有鉴于此，**偏向锁只适合于相当一大部分锁并没有被争用的系统之中**

#### 12.1.4 适应性锁

适应性锁是JIT编译器对内部锁实现所做的一种优化。存在锁争用的情况下，一个线程申请一个锁的时候如果这个锁恰好被其他线程持有，那么这个线程就需要等待该锁被释放，实现等待的一个方法就是将线程暂停——将线程的生命周期变为非Runnable状态，此时会导致上下文切换

因此，上述**暂停锁的策略适合系统中大多数线程对锁的持有时间较长的场景**，这样才能抵消上下文切换的开销；另一种实现方法就是**忙等（Busy Wait）**，也就是相当于一个循环体为空的循环语句

```java
while(lockIsHeldByOtherThread){}
```

忙等通过反复执行空操作直到所需的条件成立，这种策略的好处就是不会导致上下文切换，缺点是会消耗处理器资源，**因此忙等策略适合于系统中大多数线程对锁持有时间较短的场景**

**Java虚拟机可以综合使用上述两种策略，Java虚拟机会根据其运行过程中收集到的信息来判断这个锁是被线程长期持有还是较短持有，如果锁被较长时间持有那么会采用暂停策略；如果锁被较短时间持有，那么就可能采用忙等策略。除此之外，Java虚拟机也可能先采用忙等策略，在忙等失败下再采用暂停等待策略**

Java虚拟机的这种优化就被称为适应性锁，因此通过适应性锁可以看出，**内部锁的使用并不一定会导致上下文切换**
...
* 上下文切换与线程调度开销。线程的暂停与唤醒就是一个上下文切换的过程，并且Java虚拟机维护的等待队列也会产生一定的开销
* 内存同步、编译器优化受限的开销。内存屏障导致的冲刷写缓冲区和清空无效化队列；另外内存屏障阻碍编译器的优化
* 限制可伸缩性。锁的排他性的本质是将局部并发计算改为串行计算，锁的排他性会导致处理器资源的浪费，并限制系统的吞吐量

锁的开销主要体现再争用锁上面，**影响锁的争用程度的因素有两个：程序申请锁的频率和锁被持有的时间的跨度**，因此可以通过降低锁的争用来减少锁的开销

#### 12.2.2 可参数化锁

如果一个方法或者类内部锁使用的锁实例可以由该方法、类的客户端代码指定，那么就称这个锁是可参数化的，相应的锁称为**可参数化锁**

可参数化锁在特定情况下可以减少锁实例的个数，减少锁的开销

#### 12.2.3 减小临界区的长度

**减小临界区的长度可以减少锁被持有的时间从而降低锁被争用的概率，从而减少锁的开销，另外临界区长度的减少也有利于适应性锁发挥作用**

#### 12.2.4 减小锁的粒度

**锁的粒度：**一个锁实例所保护的共享数据的大小就被称为这个锁的粒度。一个锁所保护的共享数据的数量大就称这个锁的粒度粗，反之称这个锁的粒度小

**减少锁的粒度可以降低锁的申请频率，从而减少锁被争用的概率。例如将一个粗粒度的锁拆分成若干个粒度更细的锁**。这种技术被称为**锁拆分技术**

**不过锁的粒度过细就会增加锁的调度开销**

**锁分段技术**

锁拆分技术可以演变成一种被称为锁分段的技术。**锁分段是指对同一个数据结构内不同部分的数据使用不同的锁实例进行加锁**

**ConcurrentHashMap内部使用的就是锁分段技术**。ConcurrentHashMap内部会创建N（默认是16）个锁实例，一个线程执行put方法时提供的key参数对应的HashCode（key.hashCode()方法的返回值）会传递给一个Hash函数，该函数的返回值介于0~N-1之间，ConcurrentHashMap通过Hash函数的返回值就能确定当前线程所需要使用的锁实例。因此即使同时使用put函数，只要它们的key值不同，那么它们所使用的锁实例也不一样

#### 12.2.5 锁的替代品

例如使用volatile关键字、原子变量、无状态对象、不可变对象或者线程特有对象代替锁的使用，从而从根本上减少锁的开销

### 12.3 减少上下文切换

* 减少锁的争用或者避免锁的使用都可以减少上下文切换
* 控制线程数量。在处理器个数远小于系统所支持的并发线程数的情况下，上下文切换时保障处理器资源对各个线程访问的必要手段。因此控制线程数量能在一定程度上减少上下文切换。但是有些时候线程数量并没有直接暴露出来，例如使用Executors.newCachedThreadPool()工具类方法创建线程池时，该线程池如果没有空闲的线程用于处理新任务，那么该线程就会创建一个新的线程来处理（因为该线程池的最大线程池大小不受限）。因此极端情况下，这个线程池会创建远大于处理器数量的线程，导致过多的上下文切换从而拖慢整个系统。因此，**Executors.newCachedThreadPool()这种创建线程池的方法适合于处理任务时间较短的且不涉及阻塞操作的任务**。因为该线程池并没有提供接口来控制其最大线程池大小，因此需要**借助SemaPhore来控制并发线程的目的**
* 避免在临界区中执行阻塞式I/O等阻塞操作。临界区中的阻塞操作会增加引导这个临界区的锁被争用的可能性，从而导致增加上下文切换的可能
* 避免在临界区执行比较耗时的操作
* 减少Java虚拟机的垃圾回收。**垃圾回收可能导致所有应用程序被暂停。因为Java垃圾回收器在其工作中往往需要移动存活对象（即未被回收的对象）**。由于移动存活对象意味着这些对象的内存地址发生变化，因此在移动存活对象前垃圾回收器将所有应用程序暂停，并在移动结束后再将所有应用线程唤醒

### 12.4 多线程设计模式

### 12.5 伪共享

由于缓存行中可以存储多个变量的副本，因此即便时在两个线程各自仅访问自己的共享变量的情况下，一个线程更新其共享变量也可能导致另外一个线程访问其共享变量时产生缓存未命中，这种现象就被称为**伪共享**

这是因为，如果两个线程的特有变量都被存储在一个缓存行之中，一个处理器更新了其中一个共享变量，会导致其他处理器上包含这个共享变量副本的缓存条目被无效化（MESI协议）。此时，这个处理器上的其他线程再次访问这个被无效化的条目时就会产生缓存未命中

#### 12.5.1 Java对象内存布局

Java对象是存储在堆内存之中的。**对象的内存布局就是指对象在内存中是如何组织的，或者说是以什么样的形式在内存中呈现的**

Java对象在内存中存储包括对象头和实例字段。**对象头会使用2个字（注意不是字节，字节是计算机最小的寻址单元；字是计算机一次处理数据的最大单位。如果是32位的系统，则一个字是4个字节，如果是64位，则是8个字节）：第一个字用于存储对象的HashCode、锁相关的信息（比如偏向锁）；第二个字用于存储对象所属类的指针**

为了提高内存访问效率并减少空间浪费，Java虚拟机会依照一定的规则将一个对象的对象头和实例字段字段分配到内存空间进行存储

1. 规则1：**对象是以8字节为粒度进行对齐的**。假如有4个数据a（4个字节）,b（4个字节）,c（4个字节）,d（8个字节）。如果这4个数据按照a,b,c,d的顺序进行存储，此时a和b会占用一个内存空间（8个字节），此时c会占用半个存储空间（4个字节），但是之后的d是8个字节的，无法存入c余下的空间中，因此d会单独存一个8字节的空间，而此时c余下的4字节的空间会被**填充（padding）**

   可见，对齐往往需要填充，填充意味着空间浪费，为了减少这种浪费Java虚拟机在存储实例的时候并不是按照源代码中变量声明的顺序进行存储的

2. 对象的实例字段按照以下顺序而非源代码顺序排列

   * long和double
   * int和float
   * short和char
   * boolean和byte
   * 引用型变量（32位系统占用4个字节；64位系统占用8个字节）

3. 继承自父类的实例字段不会与类本身定义的实例字段混杂在一起进行存储

#### 12.5.2 伪共享的侦测与消除

**由于伪共享的前提是多个线程访问了位于同一缓存行中的不同变量**（尽管这些线程并没有访问同一个共享变量），因此消除伪共享的一个思路就是不让这些线程所访问的共享变量被加载到同一个缓存行中，也就是可以通过**填充**一些无用的实例变量来干扰对象的内存布局

Java 8引入了**@sun.misc.Contended注解，该注解可以用来注释类和字段，其作用是给Java虚拟机一个提示——被注释的字段或者类可能面临伪共享问题**

**Java虚拟机根据这个注释进行填充将被注释的实例变量或者类单独加载到一个缓存行之中**

目前默认情况下Contended注解仅开放给JDK内部的类使用，如果自身的类需要使用该注解需要开启Java虚拟机的开关**“-XX:RestrictContended”**
